...
mod texture_node;
use tags::{BillboardTag, FirstPass, FirstPassCube, MainPassCube};
...
impl Plugin for DynamicBillboardingPlugin {
    fn build(&self, app: &mut AppBuilder) {
        ...
        // .add_system(secondary_startup.system());
...
fn secondary_startup(
    ...
    query: Query<&Transform, With<BillboardTag>>,
) {
    println!("SEcond start");
    for transform in query.iter() {
        println!("P {}", transform.translation);
    }
}
